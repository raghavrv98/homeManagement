...
const FaridabadMisc = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      setLoading(true);
      try {
        const response = await fetch(`${API_URL}/user/raghav/faridabadMisc`, {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          alert("Faridabad Misc entry saved successfully");
          setFormData({
            name: "",
            cost: "",
  ...
  return (
    <>
      <Header backLink="/home/money" title="Faridabad Misc Expense Entry" />
      <div className="container">
        <form className="form" onSubmit={handleSubmit}>
          <Typography variant="h5" align="center" gutterBottom>
            Add Faridabad Misc Expense
          </Typography>

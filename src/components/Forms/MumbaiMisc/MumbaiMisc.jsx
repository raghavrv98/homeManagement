...
const MumbaiMisc = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      setLoading(true);
      try {
        const response = await fetch(`${API_URL}/user/raghav/mumbaiMisc`, {
          method: "POST",
          headers: {
        ...
        if (response.ok) {
          alert("Mumbai Misc entry saved successfully");
          setFormData({
            name: "",
            cost: "",
  ...
  return (
    <>
      <Header backLink="/home/money" title="Mumbai Misc Expense Entry" />
      <div className="container">
        <form className="form" onSubmit={handleSubmit}>
          <Typography variant="h5" align="center" gutterBottom>
            Add Mumbai Misc Expense
          </Typography>
